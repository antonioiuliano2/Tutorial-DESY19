...
def displayshower(df, whichshower, mycolor, axiszy, axiszx):
 '''displays with segments'''
 dfshower = df.query("MCEvent=={}".format(whichshower))
 (lcxz,lcyz) = computelines(dfshower)
 axiszy.plot(dfshower['z'], dfshower['y'],"o",fillstyle="none",color = mycolor)
 axiszx.plot(dfshower['z'], dfshower['x'],"o",fillstyle="none",color = mycolor)
...
def drawallshowers(df,nshowers,axes):
 '''draw nshowers from df with axes representing the three projections: in this order
    1) xy
    2) zy
    3) zx
 '''
 ...
 for ishower in range(nshowers):
  ...
  axes[0].plot(dfshower["x"],dfshower["y"],"o",fillstyle="none", c=color)
