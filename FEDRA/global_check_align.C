...
void checkdz(TString runname, int lastplate=57, int firstplate=1, int major = 0, int minor = 0){
 ...
 if (minor > 0 ) bricknumber = "1"; //Dario's notation does not change brick number
 cout<<bricknumber<<endl;

 TString setposition = TString("/b00000"+bricknumber+"/b00000"+bricknumber+Form(".0.%i.%i.set.root",major,minor)); //example: /b000005/b000005.0.0.0.set.root
 ...
 for (int i = firstplate; i< lastplate; i++){
  ...
  hgraphA->SetPoint(ipoint,(i+i+1)/2.,dzA);
  ipoint++;
 }
...
void align_check(TString runname,int lastplate, int firstplate, int major = 0, int minor = 0){
 TString dir = TString("/ship/DESY2019/");
 TString bricknumber = TString::Format("%d",TString(runname(3,3)).Atoi());
 if (minor > 0) bricknumber = "1";
 ...
 for (int i = firstplate; i <= lastplate; i++){
  ...
  inputfile = TFile::Open(Form((path+"AFF/"+bricknumber+".%i.%i.%i_"+bricknumber+".%i.%i.%i.al.root").Data(),i,major,minor,i-1,major,minor)); //example:AFF/2.10.0.0_2.9.0.0.al.root
...
int global_check_align(TString runname = "",int lastplate=57, int firstplate=1,int major=0,int minor=0){
 ...
 align_check(runname,lastplate,firstplate,major,minor);
 checkdz(runname,lastplate,firstplate,major,minor);
 return 0;
}
