...
TString path = "/ship/DESY2019/" + run +"/b000008/"; 
...
void couplesecography(){
 ...
 for (int iplate = firstplate; iplate <= lastplate; iplate++){
  ...
  if (iplate < 10) inputfile = TFile::Open(Form((path+"p00%i/%i.%i.0.0.cp.root").Data(),iplate,runnumber,iplate));
  else inputfile = TFile::Open(Form((path+"p0%i/%i.%i.0.0.cp.root").Data(),iplate,runnumber,iplate));
...
for (int i = firstplate; i <= lastplate; i++){
 if (i < 10) TFile *f = TFile::Open(Form((path+"p00%i/%i.%i.0.0.raw.root").Data(),i,runnumber,i));
 else TFile *f = TFile::Open(Form((path+"p0%i/%i.%i.0.0.raw.root").Data(),i,runnumber,i));
